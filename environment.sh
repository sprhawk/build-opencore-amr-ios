...
CFLAGS+="  -g -pipe -arch $ARCH -O2 -I$SDKROOT/usr/include -isysroot $SDKROOT -miphoneos-version-min=$IPHONEOS_MIN_VERSION  -F$SDKROOT/System/Library/Frameworks -g -Wall "
export CFLAGS

export CPP="$DEVELOPER/usr/bin/llvm-cpp-4.2"
CPPFLAGS=$CFLAGS
if [ X"armv7" == X"$ARCH" -o X"armv7s" == X"$ARCH" ]; then
    CPPFLAGS+=" -D__arm__ -U__i386__"
fi
export CPPFLAGS
...
export LDFLAGS="-miphoneos-version-min=$IPHONEOS_MIN_VERSION -arch $ARCH -L$SDKROOT/usr/lib -F$SDKROOT/System/Library/Frameworks -L$SDKROOT/usr/lib/system"
